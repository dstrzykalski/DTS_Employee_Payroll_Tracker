...
const collectEmployees = function() {
  ...
  const numEmployees = parseInt(prompt("Please Enter the Number of Employees:")) || 0;
  
  if (numEmployees <= 0 || isNaN(numEmployees)) {
    console.log('Please Enter a Valid Number of Employees');
  ...
  for (let i = 0; i < numEmployees; i++) {
    ...
    while (isNaN(salary) || salary <= 0) {
      ...
      salary = parseFloat(prompt(`Enter the Salary of Employee ${i + 1}:`));
    }
...
const getRandomEmployee = function(employeesArray) {
  ...
  console.log('Salary: ${randomEmployee.salary.toLocaleString("US", {style: "currency", currency: "USD"})}')
}
